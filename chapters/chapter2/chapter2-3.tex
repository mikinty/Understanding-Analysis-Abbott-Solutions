...
\bx{
\ea{
\item  We are given $(x_n) \rightarrow 0$, so we can make $\abs{x_n - 0}$ as small as we want with sufficiently large $N$.

In particular, for some $\epsilon > 0$, we choose $N$ such that $\forall n \geq N$,
\begin{align*}
	\abs{x_n - 0} < \epsilon^2
		&\implies x_n < \epsilon^2\\
		&\implies \sqrt{x_n} < \sqrt{\epsilon^2}\\
		&\implies \abs{\sqrt{x_n}} < \epsilon \tag{$\sqrt{x_n} > 0$}
...
If $x \neq 0$, then we must have $x > 0$ since $x_n \geq 0$. Now, notice
\begin{equation*}
	\abs{
		\sqrt{x_n} - \sqrt{x}
	} = \frac{
